...
object UserResourceIndex {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val tokenIndex = tokenDf.select($"token").rdd.distinct().zipWithUniqueId().map(line =>{
      (line._1.getAs[String]("token"), line._2.toInt)
    ...
    val resIdxData = resTokenTfidfData.select("resource").rdd.distinct().zipWithUniqueId()
      .map(line =>{
      (line._1.getAs[String]("resource"), line._2.toInt + maxTokenIdx) // resourceId在tokenid基础上增加
    ...
    residTokidData.withColumn("svmformat", svmformat($"resId", $"tokidTfidf"))
      .select("svmformat")
