...
import com.netease.music.recommend.scala.feedflow.tag.MixedSimVideoUnionML.{getItemFactorM, getValidVideosList}
  ...
  def main(args: Array[String]): Unit = {
    ...
    val itemFeatures = spark.read.parquet(cmd.getOptionValue("item_features")) // id, idtype, features
        .map(row => {
          ...
          val features = row.getAs[mutable.WrappedArray[Double]]("features")
          var valid = false
