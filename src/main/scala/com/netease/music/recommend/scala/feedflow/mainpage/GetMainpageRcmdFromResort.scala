...
/**
  ...
  */
object GetMainpageRcmdFromResort {
  ...
  val KWD_REC_WINDOW = 3
  val SONGART_REC_WINDOW = 3
  ...
  def main(args: Array[String]): Unit = {
    ...
    val (vidTitleM, vidIn4Cat) = getVideoInfoM(videoInput, sidVideosM, aidVideosM, videoPredM, vidSidAidM, aidsidVideosM)
    ...
    val vidIn4CatBroad = sc.broadcast(vidIn4Cat)
    ...
    // 将热门视频输出
    val hotValidVideos = flushHotVideo(vidTitleM, vidIn4Cat, hotVideos, cmd.getOptionValue("output") + "/hotvideo")
    val hotValidVideosBroad = sc.broadcast(hotValidVideos)
    println("hotValidVideosBroad size:" + hotValidVideosBroad.value.size)
    ...
    println(videoKeywordsM(keywordVideosM.get("宅舞").get(0)._1))
    ...
    val resortResultData = spark.read.option("sep","\t").csv(mlResortInput.split(",").toSeq : _*)/*.map(line => {
    ...
    val rcmdData = uidSongPref
      .join(uidProvince, Seq("uid"), "outer")
      .join(uidPrefArt, Seq("uid"), "outer")
      .join(resortResultData, Seq("uid"), "outer")
...
//        if (testUidsBroad.value.contains(uid) || uid.toLong % 2 == 0) {
          ...
          if (resortResult != null) {
            val (rcmdsForKeywordCands, guesslikeCands) = getTopKeywordVideoM(resortResult, recedVids, recedResources, recedSids, recedAidOrKeywords,
              ...
              vidTitleMBroad.value, vidIn4CatBroad.value, CAND_MAX_NUM_FOR_SINGLE_KEY, CAND_MIN_NUM_FOR_SINGLE_KEY, CAND_MAX_NUM_FOR_GUESS, rcmdsForArtCands,
              uidPcode, ridPcodeM,
              uid, hotValidVideosBroad.value)
            ...
            if (uid.equals("359792224")) {
              println("1 result size:"  + result.size)
              println(result)
            }
            ...
            if (uid.equals("359792224")) {
              println("2 result size:"  + result.size)
              println(result)
            }
...
//        }
  ...
  def getTopKeywordVideoM(resortResult: String, recedVids: mutable.HashSet[String], recedResources: mutable.HashSet[String], recedSids: mutable.HashSet[String], recedAidOrKeywords: mutable.HashSet[String],
                          ...
                          videoTitleM: mutable.HashMap[String, String], vidIn4Cat: mutable.HashSet[String],
                          ...
                          uid: String, hotValidVideos: mutable.ArrayBuffer[String]) = {
        ...
        if (!recedKwdS.contains(kwd)) {
          ...
          while (videosIter.hasNext && rcmdsForGuessCands.size < maxNum && !matchVideo) {
            ...
            if (!recedVids.contains(vid) /*!curRecedIdtype.contains(vid + ":video")*/ ) { // TODO 过滤art sid等
              //curRecedIdtype.add(vid + ":video")
              if (videoTitleM.contains(vid) && vidIn4Cat.contains(vid)) {
                recedVids.add(vid)
                val reason = rcmdvidReasonM.getOrElse(vid, null)
                if (reason != null && reason != None) {
    ...
    // 猜你喜欢补充离线结果， TODO
    if (uid.equals("359792224")) {
      println("1 user guess 359792224 ")
      println(rcmdsForGuessCands.size)
      println(rcmdsForGuessCands)
      println("vs size：" + vs.size)
    }
    for (vreason <- vs if rcmdsForGuessCands.size < maxGuessNum) {
      val rcmdResons = vreason.split("_", 2)
      val rcmdVideo = rcmdResons(0).split(":")
      // 将用户resort结果按kwd分组收集到kwdRcvideosM
      if (rcmdVideo(1).equals("video")) { // 只处理了视频
        val vid = rcmdVideo(0)
        if (!recedVids.contains(vid) && videoTitleM.contains(vid) && vidIn4Cat.contains(vid)) {
          recedVids.add(vid)
          val reason = rcmdvidReasonM.getOrElse(vid, null)
          if (reason != null && reason != None) {
            rcmdsForGuessCands.append(vid + ":video:bySortGs_" + reason)
    ...
    // 补充热门， TODO
    if (uid.equals("359792224")) {
      println("2 user guess 359792224 ")
      println(rcmdsForGuessCands.size)
      println(rcmdsForGuessCands)
      println("hotValidVideos size:" + hotValidVideos.size)
    }
    for (vidtype <- hotValidVideos if rcmdsForGuessCands.size < 1) {
      val vid = vidtype.split(":")(0)
      if (!recedVids.contains(vid)) {
        recedVids.add(vid)
        rcmdsForGuessCands.append(vid + ":video:byHotGs")
      }
    }
    if (uid.equals("359792224")) {
      println("3 user guess 359792224 ")
      println(rcmdsForGuessCands.size)
      println(rcmdsForGuessCands)
    }
  ...
  def getVideoInfoM(inputDir: String, sidVideosM: mutable.HashMap[String, mutable.ArrayBuffer[(String, Float)]],
    ...
    val vidIn4Cat = new mutable.HashSet[String]()
    val hdfs: FileSystem = FileSystem.get(new Configuration())
    ...
    if (hdfs.isDirectory(path)) {
      for (status <- hdfs.listStatus(new Path(inputDir)) ) {
        ...
        if (bufferedReader != null) {
          getVideoInfoMFromFile(bufferedReader, vidTitleM, vidIn4Cat, sidVideosM, aidVideosM, videoPredM, vidSidAidM, aidsidVidM)
  ...
  def getVideoInfoMFromFile(reader: BufferedReader, vidTitleM: mutable.HashMap[String, String], vidIn4Cat: mutable.HashSet[String],
    ...
    var line = reader.readLine()
    var match4CatSubnew = 0
    // 7_游戏 20_舞蹈 10_ACG 1_音乐资讯 16_音乐现场 7001 音乐推荐
    val validSet = Set("1", "7", "10","16", "20", "7001")
    while (line != null) {
      //      try {
      implicit val formats = DefaultFormats
      val json = parse(line)
      ...
      val artistIds = parseIds((json \ "artistIds").extractOrElse[String](""))
      val auditTagIds = parseIds((json \ "auditTagIds").extractOrElse[String](""))
      val categories = parseIds((json \ "category").extractOrElse[String]("").replace(",", "-"))
      val forNew = (json \ "forNew").extractOrElse[Int](0)
      var title = (json \ "title").extractOrElse[String]("推荐视频")
      title = title.replaceAll(",", "，").replaceAll(":", "：").replaceAll("\r", "").replaceAll("\n", "").replaceAll("《", "<").replaceAll("》", ">")
      // TODO 白名单， minFLow, 分辨率
      println("vid:" + videoId)
      var validV = true
      if (!categories.isEmpty && validSet.contains(categories(0)) || forNew == 1) {
        // validV = true
        match4CatSubnew += 1
        vidIn4Cat.add(videoId)
      }
      // TODO 周杰伦
      if (auditTagIds.contains("37028") || artistIds.contains("6452")) {
        validV = false
      }
    ...
    println("match4CatSubnew:" + match4CatSubnew)
  ...
  def flushHotVideo(vidTitleM: mutable.HashMap[String, String], vidIn4Cat:mutable.HashSet[String], hotVideos: Array[Row], outPath: String) = {
    val hdfs: FileSystem = FileSystem.get(new Configuration())
    var outpath = new Path(outPath)
    var bwriter = new BufferedWriter(new OutputStreamWriter(hdfs.create(outpath)))
    ...
    for (line <- hotVideos) {
      ...
      if (kv.length >= 2) {
        ...
        for (vidtype <- kv(1).split(",")) {
          ...
          if (vidType(1).equals("video") && vidTitleM.contains(vidType(0)) && vidIn4Cat.contains(vidType(0))) {
