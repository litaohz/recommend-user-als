...
object RcmdVideoBySims {
  ...
  def main(args : Array[String]) : Unit = {
    ...
    options.addOption("video_stats", true, "video_stats")
    ...
    println("1. 读取视频特征数据...")
    val inputVideoFeatrue = cmd.getOptionValue("video_stats")
    ...
    // TODO 300
    val videoPredM = getVideoScoreM(spark.read.parquet(inputVideoFeatrue).toDF().select("play", "idtype", "score").collect())
    //val videoPredM = getVideoPredM(viter)
    println("vieoPrefM size:" + videoPredM.size)
    println(videoPredM.head)
...
*/
    ...
    val (songid2vdieoM, artid2videoM) = getSimsM(cmd.getOptionValue("sim_als"), videoPredM)
    ...
    */
      ...
      if (userId.equals("359792224")) {
        ...
        if (songidWtM != null) {
          for ((prefsongid, wt) <- songidWtM) {
            ...
            if (simres != null) {
              for (simr <- simres) {
                val idscore = simr.split(":")
                if (!recedVids.contains(idscore(0))) {
                  val (prefset, lstWt) = simresSrcprefsM.getOrElse(idscore(0), (mutable.HashSet[String](), 0F))
                  prefset.add(prefsongid + "-song")
                  simresSrcprefsM.put(idscore(0), (prefset, lstWt + wt * idscore(1).toFloat))
                }
              }
            }
          }
        ...
        if (simresSrcprefsM != null) {
          for ((prefsongid, wt) <- songidWtM if simresSrcprefsM.size < 20) {
            if (random.nextInt(2) == 0) {
              val simres = songid2VideoML.getOrElse(prefsongid, null)
              if (simres != null) {
                val lstCnt = simresSrcprefsM.size
                for (simr <- simres if simresSrcprefsM.size < (lstCnt + 2)) {
          ...
          for ((prefaid, wt) <- aidWtM) {
            ...
            if (simres != null) {
              for (simr <- simres) {
                val idscore = simr.split(":")
                if (!recedVids.contains(idscore(0))) {
                  val (prefset, lstWt) = simresSrcprefsMArt.getOrElse(idscore(0), (mutable.HashSet[String](), 0F))
                  prefset.add(prefaid + "-artist")
                  simresSrcprefsMArt.put(idscore(0), (prefset, lstWt + wt * idscore(1).toFloat))
                }
              }
            }
          }
        ...
        */
        /*
        if (aidWtM != null) {
          for ((prefaid, wt) <- aidWtM if simresSrcprefsMArt.size < 20) {
            if (random.nextInt(2) == 0) {
              val simres = artid2videoMBroad.value.getOrElse(prefaid, null)
              if (simres != null) {
                val lstCnt = simresSrcprefsMArt.size
                for (simr <- simres if simresSrcprefsMArt.size < (lstCnt + 2)) {
                  if (!recedVids.contains(simr)) {
        ...
        */
        ...
        val rcmdResultFromSong = getRcmd(simresSrcprefsM, videoPredML, userId, 40)
        //val rcmdResultFromArt = getRcmd(simresSrcprefsMArt, videoPredML, userId, 40)
        ...
        if (rcmdResultFromSong.size > 0) {
          ...
          //} else {
          //  (2, userId + "\t" + rcmdResultFromSong.mkString(","))
          //}
        }
        /*
        if (rcmdResultFromArt.size > 0) {
          (3, userId + "\t" + rcmdResultFromArt.mkString(","))
        }*/
        else {
          (4, null)
        }
    ...
    //rcmdData.filter($"st" === 3).select("data").repartition(80).write.text(cmd.getOptionValue("output") + "/mixAlsA")
    // rcmdData.filter($"st" === 2).select("data").repartition(80).write.text(cmd.getOptionValue("output") + "/mixAls")
  ...
  /*
         ...
         */
  def getSimsM(inputDir: String, videoPredM: mutable.HashMap[String, Double]) = {
    ...
    if (hdfs.isDirectory(path)) {
      for (status <- hdfs.listStatus(new Path(inputDir)) ) {
        ...
        getSimsMFile(bufferedReader, songid2videosM, artid2videosM, videoPredM)
    ...
    (songid2videosM, artid2videosM)

  }
  def getSimsMFile(reader: BufferedReader, songid2videosM: mutable.HashMap[String, ArrayBuffer[String]], artid2videosM: mutable.HashMap[String, ArrayBuffer[String]]
                  , videoPredM: mutable.HashMap[String, Double]) = {
      ...
      if (ts.size >= 2) {
        ...
        while (iter.hasNext && simres.size < maxSimNum) {
          ...
          if (simidtype.endsWith("-video")) { // 视频默认省略后缀
            val vid = simidtype.split("-")(0)
            if (videoPredM.contains(vid)) {
              simres.append(vid + ":" + idtypeWt(1).substring(0, 3)) // 保留2位分数
            }
          } else {
            simres.append(simidtype + ":" + idtypeWt(1).substring(0, 3)) // 保留2位分数
          }
        }
  ...
  def getRcmd(simresSrcprefsM: mutable.LinkedHashMap[String, (mutable.HashSet[String], Float)], videoPredML: mutable.HashMap[String, Double],
    ...
    if (simresSrcprefsM.size > 0) {
      for ((simres, (set, wt)) <- simresSrcprefsM) {
        val pscore = videoPredML.getOrElse(simres, 0.0)
        //if (pscore >= 0.1) {
        val score = /*Math.sqrt(set.size) * */ wt * (1 + pscore) // 偏好数量 + 视频预估分
