...
/**
  ...
  */
object GetSatisfiedArtistsForFeedUser {
...
//    artists.map(_._2).reduce(_ + _)
  ...
  def main(args: Array[String]): Unit = {
    ...
    options.addOption("feedActiveUser", true, "input directory")
    ...
    val feedActiveUserInput = cmd.getOptionValue("feedActiveUser")
    ...
    // 获取动态十日活用户ID
    val feedActiveUserTable = spark.sparkContext.textFile(feedActiveUserInput)
      .map(line => {
        var userId = line.trim.split("\t")(3).toLong
        if (userId < 0)
          userId *= -1
        UserId(userId)
      })
    ...
    val userRelativeArtistsTable = feedActiveUserTable
      .join(artistSubTable, Seq("userId"), "outer")
