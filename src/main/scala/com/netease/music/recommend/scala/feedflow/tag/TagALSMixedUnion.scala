...
object TagALSMixedUnion {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 取音乐偏好正向用户  + 正向用户本身并集
    val userItemRatingDataUnion = userItemRatingData.union(userPosVideosPrefData)
    println("userItemRatingDataUnion cnt:" + userItemRatingDataUnion.count())


    val userIdMapping = userItemRatingDataUnion.select($"userIdStr").rdd.distinct().zipWithUniqueId().map(line =>{
      (line._1.getAs[String]("userIdStr"), line._2)
    }).toDF("userIdStr", "uid").cache()

    val itemMapping = userItemRatingDataUnion.select($"idtype").rdd.distinct().zipWithUniqueId().map(line =>{
      (line._1.getAs[String]("idtype"), line._2)
    }).toDF("idtype", "itemId").cache()

    val userItemRating = userItemRatingDataUnion
    ...
    val model = ALS.trainImplicit(training, 128, 5, 0.01, 1)
    ...
    println(s"Mean Squared Error = $MSE") // union = 1.2
