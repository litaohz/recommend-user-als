...
import com.netease.music.recommend.scala.feedflow.als.FiltMusicInSims.isMusicC
...
object MixMusicVideoData {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //  [10,19003] ACG:宅舞
    //  [10,15018]： ACG:唱见
    for (cat <- "1,16,7001,20,10,8001_18002,23_9001,22_18001".split(",")) {
    ...
    val userVidPrefFromEvent = spark.read.textFile(user_pref_event).map(line => {
      ...
      if (ueprefs.size >= 3) {
        ...
        if (musicVideoSetBroad.value.contains(vid)) {
          (uid, prefVideo, true)
    ...
    }).toDF("uid", "vidtypeFromEvent", "valid")
      .filter($"valid" === true)
      .groupBy($"uid").agg(
    ...
    */
    ...
    val userVidPref = spark.read.parquet(cmd.getOptionValue("user_pref_video"))
        .withColumn("isMusic", isIn(musicVideoSetBroad.value)($"videoId"))
        .filter($"isMusic" === true)
    ...
    val userPosVideosPrefData = spark.read.textFile(cmd.getOptionValue("vid_uid_pos")).map(line => {
      ...
      if (musicVideoSetBroad.value.contains(ts(0))) {
        (uid, idtype, true)
      } else {
        (uid, idtype, false)
      }
    }).toDF("uid", "vidtypePos", "isMusic")
        .filter($"isMusic" === true)
      .groupBy($"uid").agg(
    ...
    val joinData = prefData.flatMap(row => {
      ...
      val videoWtM = row.getAs[scala.collection.immutable.Map[String, Double]]("videoWtM") //updateWtM(videoImWtM, itemWtMBroad.value) // wt * idf
      //val songImWtM = row.getAs[scala.collection.immutable.Map[String, Double]]("songWtM")
      val songWtM = row.getAs[scala.collection.immutable.Map[String, Double]]("songWtM") //updateWtM(songImWtM, itemWtMBroad.value)
      //val artImWtM = row.getAs[scala.collection.immutable.Map[String, Double]]("artWtM")
      val artWtM = row.getAs[scala.collection.immutable.Map[String, Double]]("artWtM")//updateWtM(artImWtM, itemWtMBroad.value)
  ...
  def isIn(validVids: mutable.HashSet[String]) = udf((vid:String) => {
