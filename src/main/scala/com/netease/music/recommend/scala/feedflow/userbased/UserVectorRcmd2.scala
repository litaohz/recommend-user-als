...
import com.netease.music.recommend.scala.feedflow.mainpage.GetMainpageRcmdFromResort.isValid
...
/**
  ...
  */
object UserVectorRcmd2 {


  def getItemIdfM(itemCntData: Array[Row], userCnt: Long) = {
    val itemIdfM = mutable.HashMap[String, Double]()
    for (row <- itemCntData) {
      val item = row.getAs[String]("item")
      val cnt = row.getAs[Long]("cnt")
      val idf = Math.log(userCnt / (cnt + 1.0))
      itemIdfM.put(item, idf)
    }
    itemIdfM
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    */
    ...
    val userCnt = userRcedVideosData.count()
    println("userRcedVideosData cnt:" + userRcedVideosData.count())

    val itemCntData = userRcedVideosData.flatMap(row => {
      val itemUser = mutable.ArrayBuffer[(String, Int)]()
      for (video <- row.getAs[mutable.WrappedArray[String]]("rcedVideos")) {
        itemUser.append((video, 1))
      }
      itemUser
    }).toDF("item", "x").groupBy($"item").agg(count($"x").as("cnt")).collect()
    println("itemCntData size:" + itemCntData.length)
    val itemIdfM = getItemIdfM(itemCntData, userCnt)
    println("itemIdfM size:" + itemIdfM.size)
    println(itemIdfM.head)
    val itemIdfMBroad = sc.broadcast(itemIdfM)
    ...
    val clusterCandsData = userVectorData.join(userPrefVideoData, Seq("userIdStr"), "left")
      ...
      .flatMap({case (key, values) =>
        val vidWtM = mutable.HashMap[String, Double]()
          ...
          if (prefVideoWts != null) {
            for (prefVideoWt <- prefVideoWts.split(",")) {
              ...
              if (validVidsBroad.value.contains(vid)) {
                val wt = vidtypeWt(1).toDouble
                vidWtM.put(vid, vidWtM.getOrElse(vid, 0.0) + wt)
              }
            }
          }
        }
        println("for cluser:" + key )
        val result = mutable.ArrayBuffer[(String, Array[String], Array[String])]()
        if (!vidWtM.isEmpty) {
          println(vidWtM.head)
          // idf加权
          for (vid <- vidWtM.keys) {
            val idf = itemIdfMBroad.value.getOrElse(vid, 1.0)
            vidWtM.put(vid, vidWtM.getOrElse(vid, 1.0) * idf)
          }
          println(vidWtM.head)

          // TODO 除以整体偏好，消除热门
          val vidWtArray1 = vidWtM.toArray[(String, Double)].sortWith(_._2 > _._2)
          val vidWtM2 = mutable.HashMap[String, Double]()
          // val vidWtM3 = mutable.HashMap[String, Double]()
          ...
          for (i <- 0 until vidWtArray1.length * 1 / 2) {
            if (vidWtArray1(i)._2 > 40) {
              ...
              cands1.add(vid)
              // pred * wt
              vidWtM2.put(vidWtArray1(i)._1, (vidWtArray1(i)._2 * (1 + videoPredMBroad.value.getOrElse(vid, 0.0))))
              // vidWtM3.put(vidWtArray1(i)._1, (Math.sqrt(vidWtArray1(i)._2) / vidImpMBroad.value.getOrElse(vid, 2.0)).toDouble)
            }
          }

          val vidWtArray2 = vidWtM2.toArray[(String, Double)].sortWith(_._2 > _._2)
          val cands2 = mutable.ArrayBuffer[String]()
          for (i <- 0 until vidWtArray2.length) {
            cands2.append(vidWtArray2(i)._1)
          }
          /*
          val vidWtArray3 = vidWtM3.toArray[(String, Double)].sortWith(_._2 > _._2)
          ...
          for (uid <- uidSet) {
            ...
            result.append((uid, cands1.toArray[String], cands2.toArray[String]))
          }

        }
        result
      }).toDF("userIdStr", "candidatesVideos1", "candidatesVideos2")
