...
object MixedSimVideoNormDot {
  ...
  def getItemFactorM(rows: Array[Row]) = {
    ...
    for (row <- rows) {
      val idtype = row.getAs[String]("friendIdStr")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val validItemFactorsNormM = getItemFactorM(itemFactorsNorm.collect())
    ...
    val simTagData = itemFactorsNorm.map(row => {
      val srcId = row.getAs[String]("friendIdStr")
